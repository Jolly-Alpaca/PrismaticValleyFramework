...
/*         public static Color getColorFromString(string colorString)
        {
            ...
            switch (colors.Length) {
                ...
                case 3: 
                    if (tryParseColors(colors, out int[] colorValues))
                        return new Color(colorValues[0], colorValues[1], colorValues[2]);
                    break;
                case 4: 
                    if (tryParseColors(colors, out int[] colorValuesA))
        ...
        {
            colorValues = new int[colors.Length];
            for (int i = 0; i < colors.Length; i++) {
                if (!int.TryParse(colors[i], out int colorValue)) return false;
                colorValues[i] = colorValue;
            }
            return true;
        }

        private static Color getColorByName(string colorString)
        {
            switch (colorString) {
                case "Prismatic": return Utility.GetPrismaticColor();
                ...
                case "Coral": return Color.Transparent;
